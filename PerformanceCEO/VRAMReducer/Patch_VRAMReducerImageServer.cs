...
namespace PerformanceCEO.VRAMReducer
{
    [HarmonyPatch(typeof(LiveryController), "TryGetLivery")]
    class Patch_VRAMReducerImageServer
    {
        ...
        public static bool Prefix(LiveryController __instance, string aircraftType, string airlineName, out GameObject livery, ref bool __result)
        {
            try
            {
                if (PerformanceCEOConfig.DebugLogs)
                {
                    PerformanceCEO.Log($"Livery Request for {airlineName} aircraft type {aircraftType} being proccessed.");
                }

                if (PerformanceCEOConfig.VRAMReductionModuleEnabled == false || VRAMReducerManager.customAirlineNames.Contains(airlineName) == false)
                {
                    if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server Exit 1"); }
                    livery = null;
                    return true;
                }
                ...
                {
                    if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server Exit 2"); }
                    livery = null;
                    return true;
                }
                ...
                if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CP 1"); }

                livery = null;
                if (airlineName.Equals("GA") && airlineLivery != null)
                {
                    if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CP 2"); }
                    ...
                    if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server Exit 3"); }
                    __result = true;
                    return false;
                }
                else
                {
                    if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CP 3"); }
                    ...
                    if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CP 4"); }
                    foreach (string liveryPath in possibleLiveries)
                    {
                        if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CPL 5"); }
                        ...
                        if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CPL 6"); }
                        if (VRAMReducerManager.LiveryPathToAirlineNameSpecialDict[liveryPath].Item2)
                        {
                            specialLiveries++;
                        }
                        if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CPL 7"); }
                        ...
                        if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CPL 8"); }
                    }
                    ...
                    {
                        if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CP 9"); }
                        if (possibleLiveryOutput.Count == 1)
                        {
                            ...
                            if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server Exit 4"); }
                            __result = true;
                            return false;
                        }
                        if (specialLiveries == 0)
                        {
                            if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CP 10"); }
                            ...
                            if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CP 11"); }
                        }
                        else
                        {
                            if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CP 12"); }
                            ...
                            if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CP 13"); }
                        }

                        if (livery != null)
                        {
                            if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server Exit 5"); }
                            __result = true;
                            return false;
                        }
                    }
                ...
                {
                    if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server CPL 14"); }
                    if (VRAMReducerManager.aircraftTypeLiveriesPathDict[aircraftType].Count > 0)
                    {
                        ...
                        livery = VRAMReducerLiveryCreator.GetLivery(VRAMReducerManager.aircraftTypeLiveriesPathDict[aircraftType][liveryIndex], airlineName);
                        if (PerformanceCEOConfig.DebugLogs) { PerformanceCEO.Log("Image Server Exit 6"); }
                        __result = true;
                        return false;
                    }
                }
            ...
            {
                PerformanceCEO.Log($"An error occured in the Image Server. The error was {ex.Message} ({ex.InnerException.Message}).");
            }
            Debug.LogError("Could not find a suitable livery for! " + aircraftType);
            PerformanceCEO.Log("Something went very very very wrong. Error! Probably was bound to happen... :( (Exit 7)");
		    livery = null;
            __result = false;
		    return true;
        }
